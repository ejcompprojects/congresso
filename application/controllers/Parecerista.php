...
class Parecerista extends Admin {
    private $all = false;
    public function __construct(){
        parent::__construct();
        $this->load->helper('frontend_helper'); 
        $this->load->helper('modalform_helper'); 
        $this->load->model('Parecerista_model', 'parecerista_model');
    }
    public function listar_todos(){
        $this->all = true;
        $this->listar();
    }
    public function listar_analisar(){
        $this->all = false;
        $this->listar();
    }
    /**
     ...
     */
    private function listar($attribute = 'nome', $order_by = 'ASC', $quantidade = 10, $nome = '0', $inicio = 0){
        $this->load->library('pagination');
         $funcao = 'listar_analisar';
        $filtros = array();
        ...
        $num_rows = $this->parecerista_model->num_rows_listar_analisar($filtros,$this->all);

        $config['base_url'] = base_url('Parecerista/'.$funcao.'/'.$attribute.'/'.$order_by.'/'.$quantidade.'/'.$nome.'/');
        ...
        $dados['funcao'] = 'listar_analisar';
        if($this->all)
            $dados['titulo'] = 'Listar Pareceristas';
        else
       ...
       $this->load->view('html-header-admin');
        ...
        $this->load->view('listar-pareceristas', $dados);
        $this->load->view('footer-admin');
    }

       
    public function reprovar_parecerista($id){
        $this->db->where('id', $id);
        $dados['status_inscricao'] = 2;
        $resposta = $this->db->update('parecerista', $dados);
        ...
        if($resposta){
             $this->session->set_flashdata('success', 'O Parecerista '.$usuario->nome.' foi <strong>reprovado</strong> com sucesso!');
        ...
        redirect(base_url('Parecerista/listar_analisar'));
    }

    public function aprovar_parecerista($id){
       $this->db->where('id', $id);
        ...
        if($resposta){
             $this->session->set_flashdata('success', 'O Parecerista '.$usuario->nome.' foi <strong>aprovado</strong> com sucesso!');
