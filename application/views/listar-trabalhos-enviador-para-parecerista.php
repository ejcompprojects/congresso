...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('#modal').on('show.bs.modal', function (event) {
      ...
      <?php if(isset($aprovar)){ 
        ?>
        if(ativo != "Sim"){

          modal.find('#mandabala').show()
          modal.find('#form').attr("action", '<?= $aprovar['url'] ?>' + id_trabalho)
          modal.find('#mandabala').text('<?= $aprovar['label'] ?>')
        }else{
        ...
        <?php if(isset($url['reprovar'])){ 
          ?>
          modal.find('#nao_aceitar').text('<?= $aprovar['label'] ?>')
          modal.find('#nao_aceitar').attr('href', "<?= $reprovar['url'] ?>"+ id_trabalho)
          <?php
        }else{ ?>
